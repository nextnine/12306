...
## 一．引言
1. 编写目的：本概要设计文档用于给出高并发火车票购票系统的高层架构视图、关键设计策略、核心领域模型与演进决策，指导后续详细设计、开发实现与上线运维。读者包括架构、开发、测试与运维人员，以及联调相关合作方；文档重点回答“系统如何拆分、关键链路如何保证一致性与可用性、外部依赖如何治理、异常高峰如何降级与恢复”等问题。本设计与需求说明书（SRS）及测试计划建立可追溯关系：关键架构决策与策略（如限流/熔断/幂等/验签/补偿、可观测性与审计）需在测试计划中落到可执行的验证项，并在上线后通过日志、指标与演练记录形成可核验的证据。本文档不展开到接口字段级与表结构级细节，但会明确这些细化内容的产出位置与约束原则，以保证设计可实施、可验证、可演进。
2. 项目背景：春运及节假日的开售窗口会出现短时间极端峰值流量，对系统的弹性扩展、库存一致性、链路稳定性与故障快速恢复提出高要求。系统需对接铁路运行图/余票与席位资源、实名核验、支付与退款、通知触达、风控反作弊等外部系统，整体交付由委托单位、开发单位与运维单位协同完成；因此需要在架构层面显式纳入外部依赖的 SLA、限流与容量约束，并对联调失败、外部超时、回调重复/乱序等情况设定统一处理策略。针对开售窗口，系统需提前完成容量评估与压测基线，准备弹性扩容与多级降级预案（如排队/限流、只保留核心下单能力、延后非关键通知等），并通过演练与可观测性手段保障高峰期间“可用优先、正确可恢复”。在满足业务目标的同时，系统还需兼顾安全合规与可审计要求，为关键操作与交易链路提供可追踪、可复盘的证据链。
3. 定义：区间段（最小扣减单元）、预扣库存（缓存原子扣减待落库）、锁座（席位占用超时释放）、幂等键（请求唯一指纹）、排队 Token（排队查询凭据）、SLO/SLA（服务目标/协议）；关键架构决策记录在 ADR。定义部分保持与数据模型和接口字段一致，减少歧义。  
...
## 四．接口设计
### 1．外部接口
- 运行图/座席：查询接口超时 500~1000ms，失败降级缓存；数据同步任务带重试与校验和。同步过程记录数据版本与来源，异常时回退上一版本，并在高峰前完成缓存预热。[^fault] 
- 实名/证件：同步/异步校验，超时 3s，最多 3 次指数退避，必须验签。可用验证码或人工核验作为降级，并对失败次数实施频控与告警。[^fault]  
- 支付平台：创建支付单、回调通知；超时 3s，回调验签+幂等，失败重试+DLQ。回调携带版本号与签名，落库失败进入重试队列并告警，退款与补偿路径保持闭环。[^fault]  
- 短信/邮件/推送：模板化发送，超时 2s，失败重试+渠道熔断；备用通道切换。模板经过预审并落库，送达率与延迟纳入监控，渠道熔断后自动切换备用。[^fault]  
- 风控服务：评分/校验接口，超时 1s，失败时走验证码/拒绝分支。[^fault] 风控策略支持灰度发布，命中风险时记录证据链并保留人工复核通道。[^fault]  

### 2．内部接口
- 服务间通过 API Gateway（HTTP/REST）和 Kafka/RabbitMQ 事件通信；同订单/车次使用分区键保证局部有序；所有写接口携带 `idempotent_key`。接口声明契约版本与兼容策略，事件格式包含 `event_id/version/timestamp` 以便回放与幂等。[^mq]   
- 调用超时与重试：核心写链路仅一次幂等重试或不重试；查询类可短重试；统一熔断策略。超时与重试参数按接口分级配置，失败进入告警与降级路径，并记录重试次数。[^fault]  
- 接口安全：网关鉴权+签名校验；内部服务校验租户/渠道/幂等键。权限模型细化为最小权限与租户隔离，敏感字段做脱敏并记录审计日志。[^security]  
