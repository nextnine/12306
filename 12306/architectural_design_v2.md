...
4. 参考资料：
   ...
   | [DOC] | 项目任务书/合同/里程碑与验收约定 | - | 作为建设依据与验收口径输入。 |
   | [SEC] | 内部安全/加密/日志审计规范 | - | 作为账号权限、密钥管理、审计留痕的统一约束。 |
   | [EXT-01] | 铁路运行图/余票库存接口协议 | - | 约束查询/扣减/释放接口契约、错误码与 SLA。 |
   | [EXT-02] | 实名核验接口协议与验签规范 | - | 约束实名字段、验签、幂等/重放防护与 SLA。 |
   | [EXT-03] | 支付/退款/回调接口协议与验签规范 | - | 约束回调验签、幂等、对账与 SLA/容量。 |
   | [EXT-04] | 短信/站内信/Push 通知接口协议 | - | 约束触达通道的限流、回执与降级策略。 |
   | [BASE] | 历史压测报告/运营数据/容量评估 | - | 提供性能与容量基线，用于目标设定与校准。 |
