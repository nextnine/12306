...
| 1.3 | 2025-12-24 | Emma | 架构完善 | 修正服务命名、完善组件图、补充改签规则、重新设计候补多车次支持、补充消息队列架构说明 |
...
- **改签规则**:
  - 每张车票只能改签一次,改签后的车票不能再次改签
  - 通过is_rescheduled字段标记是否已改签(0否,1是)
  - 改签后生成新订单,原订单的original_order_id字段记录新订单ID
  - 新订单的rescheduled_from字段记录原订单号
  - 改签不收取手续费,rescheduled_fee字段保留用于未来扩展
  - 票价差异通过total_price字段体现,多退少补
...
#### 5.2.0 为什么同时使用Kafka和RabbitMQ?

**架构决策说明**:

我们在系统中同时引入Kafka和RabbitMQ,是基于不同的业务场景需求:

| 对比维度 | Kafka | RabbitMQ | 选择原因 |
|---------|-------|----------|---------|
| **吞吐量** | 百万级/秒 | 万级/秒 | Kafka适合高吞吐场景 |
| **延迟** | 毫秒级 | 微秒级 | RabbitMQ适合低延迟场景 |
| **消息顺序** | 分区内有序 | 队列内有序 | 两者都能保证顺序性 |
| **消息持久化** | 强 | 强 | 两者都支持持久化 |
| **延迟队列** | 不支持 | 原生支持 | RabbitMQ更适合延迟任务 |
| **消息路由** | 简单 | 复杂(Exchange) | RabbitMQ路由更灵活 |

**使用场景划分**:

**Kafka使用场景** (高吞吐、削峰填谷):
1. **订单创建事件** (order-create-topic)
   - 场景: 高峰期每秒10万+订单创建请求
   - 原因: Kafka高吞吐量,适合削峰填谷
   
2. **订单支付事件** (order-pay-topic)
   - 场景: 支付成功后触发多个下游服务(出票、通知、积分等)
   - 原因: Kafka支持多消费者组,一条消息可被多个服务消费
   
3. **库存更新事件** (inventory-update-topic)
   - 场景: 库存变化频繁,需要通知多个服务(候补、统计、监控)
   - 原因: Kafka消息持久化,支持回溯,适合数据同步
   
4. **库存回补事件** (inventory-replenish-topic)
   - 场景: 退票后触发候补处理
   - 原因: 需要保证消息不丢失,Kafka持久化能力强

**RabbitMQ使用场景** (低延迟、延迟队列):
1. **订单超时取消** (order.delay.queue)
   - 场景: 订单创建15分钟后未支付自动取消
   - 原因: RabbitMQ原生支持延迟队列,实现简单
   
2. **死信队列** (order.dlx.queue)
   - 场景: 处理失败的消息需要重试或人工介入
   - 原因: RabbitMQ的DLX机制成熟,易于实现
   
3. **实时通知** (notification.queue)
   - 场景: 支付成功、出票成功等实时通知
   - 原因: RabbitMQ延迟低,适合实时性要求高的场景

**为什么不能只用一个?**

1. **只用Kafka的问题**:
   - 不支持延迟队列,实现订单超时取消需要额外的定时任务
   - 延迟相对较高(毫秒级),不适合实时通知场景
   - 死信队列支持不够完善

2. **只用RabbitMQ的问题**:
   - 吞吐量有限,高峰期10万+QPS会成为瓶颈
   - 消息持久化性能不如Kafka,可能丢失消息
   - 不支持消息回溯,无法重新消费历史数据

**架构优势**:
- Kafka负责高吞吐、削峰填谷、数据同步
- RabbitMQ负责低延迟、延迟队列、死信处理
- 两者互补,发挥各自优势,保证系统高性能和高可用
...
CREATE TABLE t_waitlist_order (
    ...
    passenger_name VARCHAR(50) NOT NULL COMMENT '乘客姓名',
    passenger_id_card VARCHAR(18) NOT NULL COMMENT '乘客身份证',
    ticket_price DECIMAL(10,2) NOT NULL COMMENT '预付票价',
    ...
    success_train_number VARCHAR(20) COMMENT '候补成功的车次号',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX idx_user_status (user_id, status),
    INDEX idx_expire_time (expire_time, status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='候补订单主表';
```

**候补车次关联表 (t_waitlist_train)**
```sql
CREATE TABLE t_waitlist_train (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    waitlist_id BIGINT NOT NULL COMMENT '候补订单ID',
    waitlist_no VARCHAR(32) NOT NULL COMMENT '候补订单号',
    train_number VARCHAR(20) NOT NULL COMMENT '车次号',
    priority TINYINT NOT NULL COMMENT '优先级:1最高,2次之,3最低',
    status TINYINT DEFAULT 0 COMMENT '状态:0待候补,1已成功,2已失败',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX idx_waitlist_id (waitlist_id),
    INDEX idx_train_priority (train_number, priority, status),
    UNIQUE KEY uk_waitlist_train (waitlist_id, train_number)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='候补车次关联表';
```

**说明**:
- **候补订单主表**: 存储候补订单的基本信息,不包含具体车次
- **候补车次关联表**: 存储候补订单关联的多个车次(最多3个),通过priority字段标记优先级
- **一对多关系**: 一个候补订单可以关联多个车次,优先处理priority=1的车次
- **候补成功**: success_train_number字段记录最终候补成功的车次号

#### 8.2.2 候补队列 (Redis)

**候补队列 (Redis Sorted Set) - 支持多车次**

**队列设计**:
```
Key: waitlist:queue:{train_number}:{train_date}:{seat_type}
Score: 提交时间戳(毫秒)
Value: {waitlist_no}:{priority}
...
# 候补订单WAIT001选择了3个车次
# 车次G123的候补队列
waitlist:queue:G123:2025-01-20:2
  1705737600000 -> "WAIT001:1"  # 优先级1
  1705737601000 -> "WAIT002:1"
  1705737602000 -> "WAIT003:2"

# 车次G125的候补队列
waitlist:queue:G125:2025-01-20:2
  1705737600000 -> "WAIT001:2"  # 优先级2
  1705737601000 -> "WAIT004:1"

# 车次G127的候补队列
waitlist:queue:G127:2025-01-20:2
  1705737600000 -> "WAIT001:3"  # 优先级3
  1705737602000 -> "WAIT005:1"
```

**说明**：
- 使用Sorted Set保证按提交时间排序
- Score为提交时间戳，确保先到先得
- Value格式: `{waitlist_no}:{priority}`,包含候补订单号和优先级
- 每个车次+日期+座位类型维护一个独立队列
- 一个候补订单可以同时出现在多个车次的队列中,但优先级不同
...
创建候补订单主表(状态:待候补)
  ↓
创建候补车次关联记录(插入t_waitlist_train表)
  ├─ 车次1 (priority=1)
  ├─ 车次2 (priority=2)
  └─ 车次3 (priority=3)
  ↓
加入Redis候补队列(每个车次一个队列)
  ├─ 加入车次1的队列 (waitlist:queue:G123:2025-01-20:2)
  ├─ 加入车次2的队列 (waitlist:queue:G125:2025-01-20:2)
  └─ 加入车次3的队列 (waitlist:queue:G127:2025-01-20:2)
  ↓
调用支付服务预付款
  ↓
返回候补订单号和车次列表
...
从Redis队列取出第一个候补订单
  ├─ Key: waitlist:queue:{train_number}:{train_date}:{seat_type}
  ├─ 命令: ZRANGE key 0 0 WITHSCORES
  └─ 结果: "WAIT001:1" (候补订单号:优先级)
  ↓
解析候补订单号和优先级
  ├─ waitlist_no = "WAIT001"
  └─ priority = 1
  ↓
检查候补订单是否有效
  ├─ 查询t_waitlist_order表,检查status是否为0(待候补)
  ├─ 检查expire_time是否已过期
  └─ 检查是否已被其他车次候补成功
  ↓
有效性判断
  ├─ 无效(已过期/已取消/已成功) ───> 从队列移除,处理下一个
  └─ 有效 ───> 继续处理
  ↓
调用订单服务创建正式订单
  ├─ 传入: train_number, train_date, seat_type, passenger_info
...
订单创建结果判断
  ...
  │                          ├─ status = 1 (已成功)
  │                          ├─ success_time = now()
  │                          └─ success_train_number = train_number
  │                                  ↓
  │                          更新候补车次关联表
  │                          ├─ 成功车次: status = 1
  │                          └─ 其他车次: status = 2 (已失败)
...
1. **优先级顺序**: 用户选择的车次按priority排序(1最高,3最低)
2. **并行监控**: 候补订单同时加入所有选择车次的队列
3. **先到先得**: 哪个车次先有票,就先处理哪个车次的候补
4. **一次成功**: 任一车次候补成功后,从所有队列移除,其他车次不再处理
5. **公平性**: 同一车次队列内,按提交时间(score)排序,确保公平

**示例场景**:

用户提交候补订单WAIT001,选择3个车次:
- 车次G123 (优先级1)
- 车次G125 (优先级2)  
- 车次G127 (优先级3)

**场景1**: G125先有退票
- 虽然G125优先级是2,但因为先有票,所以先处理
- 候补成功后,从G123、G125、G127三个队列都移除WAIT001

**场景2**: G123和G127同时有退票
- 两个车次的候补处理并发执行
- 使用分布式锁保证只有一个能成功
- 成功后,另一个车次的处理会发现订单已成功,自动跳过
