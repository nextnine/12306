...
## 章节摘要
- 引言：目标、外部依赖、总体目标（查询/写峰值、可用性、一致性）。
- 术语与缩写：库存、幂等、排队、缩写速查。
- 用户需求：用户画像/场景、用例概览与规约要点、功能/非功能需求。
- 业务规则：改签/多票约束，排队速率、频控/灰度、外部依赖约束。
- 接口概览：接口约束、接口规范补充、请求/响应示例。
- 验收与监控：验收标准、关键监控指标。

---

## 引言
- 编写目的：阐述高并发火车票购票系统的业务/非功能需求，供架构、设计、测试、运维等角色使用，并作为后续设计/测试/验收的依据。
- 项目背景：春运/节假日流量陡增，需要零超卖、可观测、可扩展的售票系统；需与铁路运行图/实名/支付/通知/风控等系统集成。
- 定义：专用术语和缩写见“术语与缩写”章节。
- 参考资料：项目任务书/合同（如有）、行业/法规要求、外部接口协议（支付/实名/短信）、相关技术规范与标准。
...
- 运行环境：云上或数据中心，支持同城多 AZ；K8s 部署；依赖 Redis/Kafka/MySQL/ES/对象存储等。
- 条件与限制：合规要求（实名、隐私保护）、外部依赖 SLA/超时限制、上线窗口配合开售节奏、资源与容量规划约束、发布需灰度与回滚预案。
...
## 数据描述
- 静态数据：站点/车次基础、席别/座席布局、价格和退改费率、黑白名单、风控/灰度策略。
- 动态数据：余票/库存、订单/明细、支付单、票号、候补请求、通知记录、日志/审计事件。
- 数据库描述：OLTP 分库分表 MySQL；搜索/索引用 ES；缓存用 Redis；文件/报表用对象存储；事件用 Kafka/RabbitMQ。
- 数据词典：关键实体字段见“数据描述”与“接口示例”约定，完整字段/约束在数据字典（另行维护）中。
- 数据采集：运行图/座席同步、实名结果、支付回调、客户端埋点（需脱敏）、监控/日志采集用于运维与分析。
