...
3. 定义：区间段（最小扣减单元）、预扣库存（缓存原子扣减待落库）、锁座（席位占用超时释放）、幂等键（请求唯一指纹）、排队 Token（排队查询凭据）、SLO/SLA（服务目标/协议），详细部分见术语与缩写。  
4. 参考资料：项目任务书/合同（如有）；国家/行业规范（实名、安全、隐私相关）；支付/实名/短信等外部接口协议与验签规范；内部安全/加密/日志审计规范；性能与容量基线（历史压测/运营数据）。  

## 二．任务概述
1. 目标：  
   - 查询：P95 < 120ms，命中率≥95%，可用性 99.99%。[^sre]  
   - 写峰值/出队后下单：P95 < 3s，峰值 ≥20 万 TPS，可用性 99.95%。[^sre]  
   - 一致性：库存 0 超卖，订单/支付/出票最终一致（分钟级对账纠偏）。[^ddia]  
...
3. 数据库描述：OLTP 分库分表 MySQL；搜索/索引用 ES；缓存用 Redis；文件/报表用对象存储；事件用 Kafka/RabbitMQ。[^dbmq]  
...
## 五．性能需求
1. 数据精确度：金额以分为单位[^currency]；库存/票数强一致；时间使用 ISO8601（含时区）。[^iso]  
2. 时间特性：查询 P95<120ms；下单出队后 P95<3s；排队等待 P95<60s。[^sre]  
...
4. 故障处理：外部依赖超时/失败返回可诊断错误码，触发重试/降级；下单幂等返回已有结果；排队超限返回 423 并引导候补；核心异常按告警规则上报。[^fault]  
...
- 故障处理：外部依赖超时/失败需返回可诊断错误码并触发重试/降级；下单幂等返回已有结果；排队超限返回 423 并引导候补。[^fault]
...
## 参考与注释
- [^sre] 《Site Reliability Engineering》/《SRE Workbook》关于 SLO、延迟目标与错误预算的推荐。  
- [^ddia] 《Designing Data-Intensive Applications》关于事件一致性、对账与补偿的建议。  
- [^dbmq] MySQL、Elasticsearch、Redis、Kafka、RabbitMQ 官方文档对典型用途与特性的说明。  
- [^currency] 金融/支付行业接口常用最小货币单位（分）约定，见主流支付网关 API 设计指引。  
- [^iso] ISO 8601 与 RFC 3339 日期时间格式标准。  
- [^fault] Resilience4j/Hystrix 的超时、退避、熔断、降级模式及 RFC 7231 错误码语义。  
