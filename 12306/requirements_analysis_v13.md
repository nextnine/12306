...
| 1.2 | 2025-12-24 | Emma | 问题修复 | 统一验证码有效期、完善改签规则、补充用例规约、添加验收标准 |
...
- **改签规则**：
  ...
  - 票价差异处理:
    ...
    * 新票价低于原票价: 系统自动退还差价
    * 票价相同: 无需补差价
  - 改签成功后生成新订单,原订单标记为"已改签"状态并关联新订单ID
  - 改签不收取手续费
...
**验收标准**:
- 所有HTTP请求必须强制跳转到HTTPS,SSL证书有效期>30天
- 敏感数据加密率100%,使用AES-256加密算法
- 密码加密使用bcrypt,工作因子≥10
- 数据库访问必须通过专用账号,禁止使用root账号
- 全量备份每日凌晨3点执行,备份成功率>99.9%
- 增量备份间隔≤5分钟,RPO(恢复点目标)<5分钟
- 异地备份延迟<30分钟,备份数据保留期≥90天
- 定期进行备份恢复演练,RTO(恢复时间目标)<1小时
...
**验收标准**:
- DDoS防护: CDN能够抵御100Gbps以上的DDoS攻击,攻击发生时正常用户访问成功率>95%
- SQL注入防护: 所有数据库操作使用预编译语句,代码审查SQL注入漏洞检出率100%
- XSS防护: 所有用户输入输出进行HTML编码,CSP策略覆盖率100%,XSS漏洞扫描通过率100%
- CSRF防护: 所有状态变更操作必须验证CSRF Token,Token有效期≤30分钟
- 暴力破解防护: 密码错误5次触发验证码,错误10次锁定账号30分钟,登录成功率>99%
- 刷票防护: 单IP每秒请求限制≤10次,单用户每分钟下单限制≤3次,设备指纹识别准确率>95%
- 安全漏洞: 每季度进行渗透测试,高危漏洞修复时间≤24小时,中危漏洞≤7天
...
**验收标准**:
- 监控覆盖率: 所有核心服务监控覆盖率100%,监控指标采集间隔≤1分钟
- 系统监控阈值: CPU使用率>80%触发警告,>90%触发严重告警; 内存使用率>85%告警; 磁盘使用率>80%告警
- 应用性能监控: 接口响应时间P99>3秒告警,错误率>1%告警,QPS异常波动>50%告警
- 数据库监控: 慢查询(>1秒)自动记录,连接数>80%告警,主从延迟>5秒告警
- 缓存监控: Redis命中率<90%告警,内存使用率>80%告警
- 业务监控: 订单创建失败率>1%告警,支付成功率<99%告警,候补处理延迟>5秒告警
- 告警响应: 紧急告警5分钟内响应,严重告警15分钟内响应,警告告警1小时内响应
- 告警准确率: 误报率<5%,漏报率<1%
- 监控数据保留: 实时数据保留7天,聚合数据保留90天
...
6a. 身份认证接口调用失败
- 6a1. 系统提示"认证服务繁忙,请稍后重试"
- 6a2. 用例结束

6b. 身份信息验证失败
- 6b1. 系统提示"身份信息验证失败,请确认信息正确"
- 6b2. 返回步骤2

6c. 该身份证已关联5个账号
- 6c1. 系统提示"该身份证已关联5个账号,无法继续认证"
- 6c2. 用例结束
...
1. 用户进入常用乘车人管理页面
2. 用户点击"添加乘车人"按钮
3. 用户输入乘车人信息(姓名、证件类型、证件号、手机号、旅客类型)
4. 系统验证输入格式
5. 系统调用实名认证接口验证身份信息
6. 系统保存乘车人信息
7. 系统返回添加成功信息
...
4a. 用户已有5个监控任务
- 4a1. 系统提示"余票监控任务已达上限(5个)"
- 4a2. 用例结束

6a. 监控任务创建失败
- 6a1. 系统提示"监控任务创建失败,请重试"
- 6a2. 用例结束

**余票变化通知流程**:
8a. 监控到有余票
- 8a1. 系统发送通知(短信/推送)
- 8a2. 通知内容包含车次、日期、座位类型、余票数量
- 8a3. 系统从监控任务列表移除该任务
...
**补充说明**:
- 余票监控是P2优先级功能,高峰期可能被降级
- 监控任务可以随时取消
- 收到通知后需要尽快购票,余票可能很快被抢完
...
3a. 订单不满足改签条件
- 3a1. 系统提示具体原因(已改签过/距离开车<48小时/订单状态不允许)
- 3a2. 用例结束

6a. 新车次无余票
- 6a1. 系统提示"该车次暂无余票,请选择其他车次"
- 6a2. 返回步骤5

6b. 新座位类型低于原座位类型
- 6b1. 系统提示"只能改签到同等级或更高等级座位"
- 6b2. 返回步骤5
...
**接口**: `PUT /api/passenger/update/{passenger_id}`
...
```json
{
  ...
  "data": {
    "passenger_id": 888999,
    "update_time": "2025-01-24T11:00:00Z"
  }
}
```

**接口**: `DELETE /api/passenger/delete/{passenger_id}`

**请求参数**:
```
passenger_id: 888999 (路径参数)
```
...
**请求参数**:
```
status: monitoring,expired,cancelled (可选)
```
...
**接口**: `DELETE /api/monitor/cancel/{monitor_id}`

**请求参数**:
```
monitor_id: 777888 (路径参数)
```
