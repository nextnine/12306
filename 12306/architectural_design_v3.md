...
### 合规与标准（需要在需求/设计/验收中显式标注）

- **个人信息保护**：用户注册/实名校验、乘车人信息、订单与行程、通知与联系人等个人信息处理要求，对齐 **GB/T 35273—2020**（最小必要、告知同意、访问控制、脱敏等）。fileciteturn42file14L11-L16  
- **移动端 App 上线测评**：若提供移动端 App，按 **GB/T 42582—2023** 做个人信息安全测评（权限、SDK、隐私政策一致性等）。  
- **支付链路安全**：支付鉴权、回调、账务、对账、审计与风控等总体框架参考 **GB/T 31502—2015**。fileciteturn42file12L6-L11  
- **网络支付数据安全**：支付数据（支付要素、交易明细、对账/账务、风控日志）分类分级与存储/传输/使用控制参考 **GB/T 42015—2022**。  
- **支付设施验收**：如涉及“非银行支付机构支付业务设施”建设/托管，检测与验收参考 **GB/T 41463—2022**。fileciteturn42file0L12-L14  
- **密码能力对接**：如采用国密算法/密码机/统一密码服务，接口与验证参考 **GB/T 43578—2023**。fileciteturn42file3L3-L6  
...
| 标识 | 来源 | 说明（在本文使用位置） |
| --- | --- | --- |
| [GUIDE-NJU] | 《南京大学软件文档编写指南》 | 文档结构、章节编号、图表与“术语/参考资料”呈现方式的模板依据。fileciteturn42file8L25-L40 |
| [BOOK-SRE] | 《Site Reliability Engineering》 | SLO/SLA、错误预算、容量与可靠性工程方法。 |
| [BOOK-SREWB] | 《The Site Reliability Workbook》 | 以实操视角落地 SLO、告警、容量规划与事故复盘。 |
| [BOOK-DDIA] | 《Designing Data-Intensive Applications》 | 一致性、分布式事务/账本化、消息与存储设计。 |
| [STD-35273] | GB/T 35273—2020《信息安全技术 个人信息安全规范》 | 个人信息全生命周期处理要求（采集最小化、告知同意、访问控制、脱敏/匿名化等）。fileciteturn42file14L11-L16 |
| [STD-42582] | GB/T 42582—2023《信息安全技术 移动互联网应用程序(App)个人信息安全测评规范》 | 移动端 App 上线/验收的个人信息合规测评参考（权限、SDK、隐私政策一致性等）。 |
| [STD-31502] | GB/T 31502—2015《信息安全技术 电子支付系统安全保护框架》 | 电子支付系统安全框架：安全问题/安全目的/安全功能与保障需求；指导支付链路安全与审计。fileciteturn42file12L6-L11 |
| [STD-42015] | GB/T 42015—2022《信息安全技术 网络支付服务数据安全要求》 | 网络支付服务的数据安全要求（对支付数据分类分级、存储/传输/使用的控制与留痕）。 |
| [STD-41460] | GB/T 41460—2022《非银行支付机构支付业务设施技术要求》 | 若系统自建/托管“支付业务设施”（账户、清结算、风险监控等），其设施能力与安全要求参考。 |
| [STD-41463] | GB/T 41463—2022《非银行支付机构支付业务设施检测规范》 | 设施检测/验收依据：启动要求、功能/风控反洗钱、性能与安全性检测。fileciteturn42file0L12-L14 |
| [STD-43578] | GB/T 43578—2023《信息安全技术 通用密码服务接口规范》 | 统一密码服务接口的数据结构/接口描述/函数定义要求及验证方法；用于密码模块/密码机/国密服务对接。fileciteturn42file3L3-L6 |
| [STD-36964] | GB/T 36964—2018《软件开发成本度量规范》 | 研发成本度量口径与方法（用于预算、里程碑与验收成本口径对齐）。fileciteturn42file16L7-L10 |
| [ISO-4217] | ISO 4217 | 金额最小货币单位（分/cent）与币种代码。 |
| [ISO-8601] | ISO 8601:2019 | 日期时间格式。 |
| [RFC-3339] | RFC 3339 | 互联网时间戳/时区表示。 |
| [RFC-HTTP] | RFC 9110 / RFC 7231 | HTTP 语义、超时、重试、错误码语义。 |
## 模型与视图
